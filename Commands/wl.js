...
module.exports.run = async (client, message, args) => {
    ...
    if(!message.mentions.members.first()){
        if(!args[0]){
            ...
            message.guild.channels.create(target.user.username, {type: "text", permissionOverwrites: [
                ...
                channel.send(new Discord.MessageEmbed() 
                    ...
                    .setDescription(`Vous disposez maintenant de votre propre salon privé pour faire des publicités avec Wolbus  !
                    Rappelez-vous que vous devez avoir un ou plusieurs tokens de bot pour utiliser Wolbus , si vous n'avez pas encore de token, adressez-vous à un helper pour en savoir plus.`)
                    .setColor(config.embedColor)
                )
    
                channel.send(new Discord.MessageEmbed() 
                    .setTitle("Utilisation de Wolbus Dm all")
                    .setDescription("__Voici les commandes que vous pouvez utiliser avec Wolbus :__")
        ...
        message.guild.channels.create(target.user.username, {type: "text", permissionOverwrites: [
            ...
            target.roles.add(config.wlrole)
            channel.send(new Discord.MessageEmbed() 
                .setTitle(`**Voici votre Salon privé, ${target.user.username}**`)
                .setDescription(`Vous disposez maintenant de votre propre salon privé pour faire des publicités avec Wolbus Dm all!
                Rappelez-vous que vous devez avoir un ou plusieurs tokens de bot pour utiliser Wolbus Dm all, si vous n'avez pas encore de token, adressez-vous à un helper pour en savoir plus.`)
                .setColor(config.embedColor)
            )

            channel.send(new Discord.MessageEmbed() 
                .setTitle("Utilisation de Wolbus")
                .setDescription("__Voici les commandes que vous pouvez utiliser avec Wolbus Dm all:__")
                .addField(config.prefix + "**pub token_bot**", "Cette commande sert à envoyer un message à tous les membres d'un serveur.")
                .addField(config.prefix + "**puball token_bot**", "Cette commande sert à envoyer un message à tous les membres des serveurs.")
                .addField(config.prefix + "pub* token_bot", "Cette commande sert à envoyer un message à tous les membres en ligne d'un serveur.")
                .addField(config.prefix + "puball* token_bot", "Cette commande sert à envoyer un message à tous les membres en ligne des serveurs.")
